...
This section describes the minimal workflow requirements and task transitions for both the coordination task and the contributor task. Except were specifically stated, this IG does not disallow other states or transitions. However, implementers should not expect other systems to recognize or understand variances from this process without prior agreement.
...
10. **Cancelled** – The Coordination Platform **SHALL** set a GFE Contributor Task status to cancelled based on the events in the GFE Coordination Workflow.
    ...
    2. The Coordination Requester **MAY** change the status of any open Coordination Task to `cancelled` if the status is not already `rejected` and **SHOULD** update the Contributor Task `statusReason`. 
    3. The Coordination Requester **SHALL NOT** change the status of a Coordination Task to `cancelled` if the status is `rejected`, and **SHALL NOT** update the `statusReason` of such a Contributor Task.
...
- For each associated [GFE Contributor Task](StructureDefinition-davinci-pct-gfe-contributor-task.html) that does not have a status of `completed` and does not have a [GFE Bundle](StructureDefinition-davinci-pct-gfe-bundle.html) in output.valueAttachment, there **SHALL** be a [GFE Missing Bundle](StructureDefinition-davinci-pct-gfe-missing-bundle.html).
...
When deemed appropriate, the GFE Coordination Requester **SHALL** close the [GFE Coordination Task](StructureDefinition-davinci-pct-gfe-coordination-task.html).status by updating the status to `completed`, `cancelled`, or `failed` and **SHALL** include a statusReason to close the request when no new GFE Contributions will be needed or accepted.
