...
6. GFE processing completes successfully and the payer produces an AEOB and related resources. 
  * The payer system sets the status of the request (identified by the URL returned in step 3) to completed, prepare a batch-response Bundle resource, and move to Step 7 below. 
  * If the payer system supports API access for the patient, the AEOB is made accessbile per the Patient Perspective section below. 
7. The payer system provides an endpoint at the URL provided in Step 3 whereby the provide can poll for GFE processing status.   
